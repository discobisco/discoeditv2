...
CANDIDATES = ["2k25_roster_editor_stage4_players_gear_hotzones_scrolled.py",
    ...
    "2k25_roster_editor_stage3_players_badges_grouped_scrolled.py",
    "2k25_roster_editor_stage3_players_dropdowns_scrolled.py",
    "2k25_roster_editor_stage3_players_scrolled.py",
    "2k25_roster_editor_stage2_scrolled.py"]



def _find_base_script2():
    """Robust base editor locator.
    Accepts any Stage 2/3/4 editor file, including *_scrolled names and variants.
    Prefers Stage 4 > Stage 3 > Stage 2; then by specificity and size.
    """
    ...
    for f in os.listdir(here):
        if not f.endswith('.py'):
            continue
        if f.startswith('2k25_stage5_'):
            continue
        if 'stage5' in f.lower():
            continue
        if not (f.startswith('2k25_roster_editor_stage') or f.startswith('2k25_player_patcher_gui')):
            continue
        full = os.path.join(here, f)
        # score
        score = 0
        if 'roster_editor' in f: score += 50
        import re as _re
        m = _re.search(r'stage(\d+)', f)
        if m:
            score += int(m.group(1)) * 10
        if 'gear' in f or 'hotzones' in f: score += 4
        if 'players' in f: score += 3
        if 'dropdown' in f or 'badges' in f: score += 2
        if 'scrolled' in f: score += 1
        if 'patcher' in f: score += 5
        if 'launcher' in f: continue
        size = os.path.getsize(full)
        mtime = os.path.getmtime(full)
        candidates.append((score, size, mtime, full))
    if candidates:
