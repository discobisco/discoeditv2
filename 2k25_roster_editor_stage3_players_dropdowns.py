...
# -*- coding: utf-8 -*-
"""
NBA 2K25 Roster Editor - Stage 3.1 (Players-first + Dropdowns)
Windows-only. Live memory editing while NBA2K25 runs offline (EAC disabled).

Adds dropdown editors for:
- Position, Secondary Position, Play Initiator (Yes/No).
- Play Type 1..4 with 0..15 mapping from CE table.
- Badges with levels: Unequipped, Bronze, Silver, Gold, Hall Of Fame, Legend.

Source of truth for offsets: offsets.json (preferred). Fallbacks: Offsets.txt, unified_offsets*.json|text, potion.txt.

Run:
    python 2k25_roster_editor_stage3_players_dropdowns.py
"""
...
class CategoryGrid(ttk.Frame):
    def __init__(self, parent, gm: GameMemory, category_name: str,
        ...
        self.current_dropdown: Optional[Dict[str, int]] = None  # label->code map for selected field
        ...
        # Combobox is not packed by default; we pack/unpack dynamically
    ...
    # Dropdown definition for a given field name
    def _dropdown_for_field(self, name: str) -> Optional[List[str]]:
        n = name.lower()
        if self.cat == "Vitals":
            if n == "position" or n == "secondary position":
        ...
        if self.cat == "Badges":
            return BADGE_LEVELS[:]  # all badges share same levels
        return None
    ...
    def _on_select(self, _e=None):
        ...
        # Install dropdown if applicable
        choices = self._dropdown_for_field(name)
        if choices:
            # Replace entry with combobox
            try:
                self.ent_val.pack_forget()
            except Exception:
                pass
            ...
            self.current_dropdown = {label: i for i, label in enumerate(choices)}
        else:
            # ensure combobox not shown
            try:
    ...
    def write_selected(self):
        ...
        if self.current_dropdown:
            disp_in = self.cmb_val.get()
            self.sel_val.set(disp_in)
        else:
            disp_in = self.sel_val.get()
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"NBA 2K25 Roster Editor - Stage 3.1 v{APP_VERSION}")
        self.geometry("1250x840"); self.minsize(1040, 680)
