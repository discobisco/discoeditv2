...
# -*- coding: utf-8 -*-
"""
NBA 2K25 Roster Editor — Stage 5.2 (P0 + Sorting + Draft Class Builder)
Windows-only. Requires an existing editor script (Stage 3.x or Stage 4) and offsets.json.

Adds on top of Stage 5.1:
- Draft Class Builder: create a list from flags or team grouping and show it in Players tab.
  Defaults (safe):
    * Is Draft Prospect == 1
    * Was Drafted == 0
  Optional checks:
    * Round Drafted == 0
    * Draft Pick Number == 0
    * Team name contains 'draft'

Also includes:
- Undo/Redo patch (Ctrl+Z / Ctrl+Y)
- Batch Ops
- Sorting
"""
...
class DraftClassTool(tk.Toplevel):
    def __init__(self, master, app, base_mod):
        super().__init__(master)
        self.title("Stage 5 — Draft Class Builder")
        ...
        # Offsets relative to player base. Bit fields as per CE tables.
        #  +0x105: [bit0 IsGenerated] [bit1 IsDraftProspect] [bit2 WasDrafted]
        ...
        #  +0x2E3 bits 3..6 : Round Drafted (binary-coded round) ; treat 0 as undrafted
        self.SPEC_ROUND_DRAFT = self._spec_from_json("Round Drafted") or (0x2E3, 3, 4)
        #  +0x2F2 bits 6..11 : Draft Pick Number
...
class Stage5Window(tk.Toplevel):
    def __init__(self, master, app, base_mod, undo_mgr):
        ...
        ttk.Label(frm, text="Draft Class").grid(row=7, column=0, sticky="w", pady=(10,0))
        ttk.Button(frm, text="Open Draft Builder", command=self._open_draft).grid(row=8, column=0, sticky="w", pady=4)
    ...
    def _open_draft(self):
        try:
            DraftClassTool(self.app, self.app, self.base)
        except Exception as e:
            messagebox.showerror("Draft Builder error", str(e))
